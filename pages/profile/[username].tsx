...
const ProfilePage: NextPage = () => {
  ...
  async function loadProfileData() {
    ...
    const [statsResponse, postsResponse, mutualResponse] = await Promise.all([
      fetch(`/api/instagram/follows?username=${encodeURIComponent(username)}&viewer=${encodeURIComponent(currentUser ?? '')}`),
      fetch(`/api/instagram/posts?username=${encodeURIComponent(username)}&viewer=${encodeURIComponent(currentUser ?? '')}&limit=30`),
      fetch(`/api/instagram/follows?username=${encodeURIComponent(username)}&viewer=${encodeURIComponent(currentUser ?? '')}&list=mutual&limit=8`)
    ])

    if (!statsResponse.ok || !postsResponse.ok || !mutualResponse.ok) {
      setLoading(false)
      throw new Error('Failed to load profile')
    }

    const [statsData, postsData, mutualData] = await Promise.all([
      statsResponse.json(),
      postsResponse.json(),
      mutualResponse.json()
    ])
    ...
    setMutualCount(Array.isArray(mutualData.users) ? mutualData.users.length : 0)
    setLoading(false)
  }
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 via-rose-500 to-orange-400 text-2xl font-bold text-white shadow-sm">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                ...
                {currentUser && currentUser === username ? (
                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">Your profile</span>
                ) : null}
              </div>
              <p className="text-sm text-slate-500">Personal profile and posts.</p>
            </div>
          </div>
          ...
          {currentUser && currentUser !== username ? (
            <button
              ...
              className={`inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-semibold transition ${
                stats?.isFollowing ? 'bg-slate-100 text-slate-900 hover:bg-slate-200' : 'bg-blue-600 text-white hover:bg-blue-700'
              }`}
            >
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 text-center md:grid-cols-4">
          <button className="rounded-2xl border border-slate-100 bg-slate-50 p-3 transition hover:-translate-y-0.5 hover:shadow-sm">
          ...
          <Link
            ...
            className="rounded-2xl border border-slate-100 bg-slate-50 p-3 transition hover:-translate-y-0.5 hover:shadow-sm"
          >
          ...
          <Link
            href={`/profile/${encodeURIComponent(username)}/mutual`}
            className="rounded-2xl border border-slate-100 bg-slate-50 p-3 transition hover:-translate-y-0.5 hover:shadow-sm"
          >
            <p className="text-2xl font-semibold text-slate-900">{mutualCount}</p>
            <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Mutual</p>
          </Link>
        </div>
      </section>
